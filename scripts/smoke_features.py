#!/usr/bin/env python3
"""
Smoke test for feature engineering.

Builds features for AAPL and verifies rows exist in features_tft.
...
async def verify_rows() -> int:
    ...
    count = await conn.fetchval(
        "SELECT COUNT(*) FROM features_tft WHERE ticker = 'AAPL'"
    )
    await conn.close()
    return count
...
async def ensure_market_data() -> int:
    """Ensure we have market data to build features from."""
    dsn = (
        f"postgresql://{_env('POSTGRES_USER', 'awet')}:{_env('POSTGRES_PASSWORD', 'awet')}"
        f"@{_env('POSTGRES_HOST', 'localhost')}:{_env('POSTGRES_PORT', '5433')}"
        f"/{_env('POSTGRES_DB', 'awet')}"
...
def main() -> int:
    print("=" * 60)
    print("SMOKE TEST: Feature Engineering")
    print("=" * 60)
    ...
    cmd = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "feature_engineer.py"),
        "--symbols", "AAPL",
        "--start", "2024-01-01",
        "--end", "2024-01-10",
        "--source", "day",
    ...
    if result.returncode != 0:
        print(f"❌ Feature engineering failed (exit {result.returncode})")
        print(result.stderr)
        return 1
    ...
    # Verify rows
    print("\n[3/3] Verifying rows in features_tft...")
    try:
    ...
    if count >= 1:
        print(f"✅ SMOKE_FEATURES_PASS: {count} AAPL rows in features_tft")
        return 0
    else:
        print(f"❌ SMOKE_FEATURES_FAIL: No AAPL rows found")
        return 1
